    ...
    static class Program
    {
        ...
        private static void CheckAndInstallToSystemPath()
        {
            ...
            try
            {
                ...
                DebugLogger.Log($"[설치] 현재 실행 경로: {currentExePath}");

                if (string.IsNullOrEmpty(currentExePath))
                {
                    DebugLogger.Log("[설치] 경로가 비어있음 - 종료");
                    return;
                }
                ...
                DebugLogger.Log("[설치] 현재 프로세스 종료");
                Environment.Exit(0);
            }
            catch (Exception ex)
            {
                DebugLogger.Log($"[설치] 오류 발생: {ex.GetType().Name}");
                DebugLogger.Log($"[설치] 오류 메시지: {ex.Message}");
                DebugLogger.Log($"[설치] 스택 트레이스:\n{ex.StackTrace}");
        ...
        static void Main()
        {
            ...
            DebugLogger.Log("=== AppLauncher 시작 ===");
        ...
        private static void KillExistingProcessesAtPath(string targetPath)
        {
            try
            {
                ...
                foreach (var process in processes)
                {
                    try
                    {
                        ...
                        {
                            DebugLogger.Log($"[설치] 프로세스 종료: PID={process.Id}, Path={processPath}");
                            process.Kill();
                            process.WaitForExit(5000); // 5초 대기
                        }
                    }
                    ...
                    {
                        DebugLogger.Log($"[설치] 프로세스 종료 실패: {ex.Message}");
                    }
                }
            ...
            {
                DebugLogger.Log($"[설치] KillExistingProcessesAtPath 오류: {ex.Message}");
            }
        }
